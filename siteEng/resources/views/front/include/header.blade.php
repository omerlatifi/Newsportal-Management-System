        ...
        <div class="row">
            <div class="col-md-4">
                <div class="DateTime h-100 d-flex align-items-center ">
                    <p class="date">
                        <i class="fa fa-map-marker" aria-hidden="true"></i> Dhaka, &nbsp;{{ date('l') }}<br>
            ...
            <div class="col-md-4 text-end">
                <div class="HeaderTopSocial h-100 d-flex align-items-center justify-content-end">
                    <div class="DSocialLink">
                        <ul>
                            <li><a href="{{ $ins_url }}" class="BanglaButton">Bangla</button></a></li>
...
@if(count($subcatList) == 0)
                                <li class="nav-item"><a class="nav-link" href="{{ route('news',base64_encode($headerCats->id)) }}">{{ $headerCats->eng_name }}</a>
                                </li>
                                @else
                                <li class="nav-item dropdown">
                                    <a class="nav-link dropdown-toggle" href="{{ route('news',base64_encode($headerCats->id)) }}"
                                       ...
                                       aria-expanded="false">{{ $headerCats->eng_name }}</a>
                                    <ul class="dropdown-menu" aria-labelledby="navbarDropdown">

                                        @foreach($subcatList as $subcatLists)
                                        <li><a class="dropdown-item" href="{{ route('subCategoryNews',base64_encode($subcatLists->id)) }}">{{ $subcatLists->eng_name }}</a></li>
                                        @endforeach
