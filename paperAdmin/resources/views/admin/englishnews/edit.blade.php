@extends('admin.master.master')

@section('title')
Update News
@endsection


@section('css')

@endsection

@section('body')
<div class="content-body">
    <div class="container-fluid">
         <div class="page-titles">
    <div class="d-flex align-items-center">
        <div class="me-auto">
            <h4 class="page-title">Update News</h4>
        </div>

    </div>
</div>
@include('flash_message')
<section class="content">
    <form class="custom-validation" action="{{ route('englishNews.update',$newList->id) }}" method="post" enctype="multipart/form-data" id="form" data-parsley-validate="">
        @csrf
        @method('PUT')
    <div class="row">
        <div class="col-lg-6 col-sm-12">
            <div class="card">
                <div class="card-body">

                        <div class="row">

                            <?php
                            $dt = new DateTime();
                            $dt->setTimezone(new DateTimezone('Asia/Dhaka'));
                            $created_at = $dt->format('h:i:sa');


?>


                            <div class="col-lg-12 col-sm-12">
                                <div class="form-group">
                                    <label class="fw-700 fs-16 form-label">Title <span style="color:red;">*</span></label>
                                    <input type="text" value="{{ $newList->title }}" required class="form-control" name="title" placeholder="Title">
                                </div>
                            </div>

                            <div class="col-lg-12 col-sm-12">
                                <div class="form-group">
                                    <label class="fw-700 fs-16 form-label">Sub-Title</label>
                                    <input type="text" value="{{ $newList->subtittle }}"  class="form-control" name="subtittle" placeholder="Sub-Title">
                                    <input type="hidden" required class="form-control" name="publish_site" value="0" placeholder="Sub-Title">
                                    <input type="hidden" required class="form-control" name="trash_status" value="0" placeholder="Sub-Title">
                                </div>
                            </div>


                            </div>

                        </div>

                </div>
            </div>

        <div class="col-lg-6 col-sm-12">
            <div class="card">
                <div class="card-body">

                    @if($newList->video_news == 1)
                    <div class="col-lg-12 col-sm-12">
                    <div class="form-group">
                        <label class="fw-700 fs-16 form-label">Youtube Video Link <span style="color:red;">*</span></label>
                        <input type="text" required class="form-control" value="{{ $newList->image_caption }}" name="image_caption" placeholder="Youtube Video Link">
                    </div>
                    </div>
                    @else
                    <div class="category_demo text-start">
                        <img id ="blah" style="height: 75px;" src="{{ asset('/') }}{{ $newList->image }}" alt=""
                             class="mb-15">
                        <div class="mb-3">
                            <label class="form-label">Product Images <span style="color:red;">*</span></label>
                            <small class="text-danger">(Ratio 1:1)</small>
                            <input name="image"  class="form-control" accept="image/*" type='file' id="imgInp" >
                        </div>
                    </div>

                    <div class="col-lg-12 col-sm-12">
                        <div class="form-group">
                            <label class="fw-700 fs-16 form-label">Image Caption <span style="color:red;">*</span></label>
                            <input type="text" required class="form-control" value="{{ $newList->image_caption }}" name="image_caption" placeholder="Image Caption">
                        </div>
                    </div>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-lg-12 col-sm-12">

            <div class="card">
                <div class="card-body">


                    <div class="row">
                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group">
                                <label class="fw-700 fs-16 form-label">Category <span style="color:red;">*</span></label>
                                <select name="category"  id="category_id" class="form-control select2" style="width: 100%;" required>
                                    <option value="">--please select --</option>
                                    @foreach($categoryList as $categoryLists)
                                    <option value="{{ $categoryLists->id }}" {{ $newList->category  == $categoryLists->id ? 'selected':'' }} data-name="{{ $categoryLists->category_name }}">{{ $categoryLists->category_name }}</option>
                                    @endforeach

                                </select>
                            </div>
                        </div>
                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group">
                                <label class="fw-700 fs-16 form-label">Sub Category </label>
                                <select name="sub_category" id="subcategory_id" class="form-control select2" style="width: 100%;">
                                    <option value="">--please select --</option>
                                    @foreach($subcategoryList as $categoryLists)
                                    <option value="{{ $categoryLists->id }}" {{ $newList->sub_category  == $categoryLists->id ? 'selected':'' }} data-name="{{ $categoryLists->subcategory_name }}">{{ $categoryLists->subcategory_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>

                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group">
                                <label class="fw-700 fs-16 form-label">Head Line <span style="color:red;">*</span></label>
                                <select name="headline_type" id="headline_type_id" class="form-control select2" style="width: 100%;" required>
                                    <option value="no" {{ $newList->headline_type  == 'no' ? 'selected':'' }}>No</option>
                                    <option value="yes" {{ $newList->headline_type  == 'yes' ? 'selected':'' }}>Yes</option>
                                </select>
                            </div>
                        </div>


                        <div class="col-lg-6 col-sm-12">
                            <div class="form-group">
                                <label class="fw-700 fs-16 form-label">View In Home Page <span style="color:red;">*</span></label>
                                <select name="home_page_position" id="home_page_position" class="form-control select2" style="width: 100%;" required>
                                    <option value="other" {{ $newList->home_page_position  == 'other' ? 'selected':'' }}>Other Section</option>
                                    <option value="first" {{ $newList->home_page_position  == 'first' ? 'selected':'' }}>First Section</option>
                                    <option value="second" {{ $newList->home_page_position  == 'second' ? 'selected':'' }}>Second Section</option>
                                    <option value="third" {{ $newList->home_page_position  == 'third' ? 'selected':'' }}>Third Section</option>
                                    <option value="fourth" {{ $newList->home_page_position  == 'fourth' ? 'selected':'' }}>Fourth Section</option>
                                </select>
                            </div>
                        </div>

                        <div class="col-lg-12 col-sm-12">
                            <div class="form-group">
                                <label class="fw-700 fs-16 form-label">Status <span style="color:red;">*</span></label>
                                <select name="status" id="status" class="form-control select2" style="width: 100%;" required>
                                    <option value="1" {{ $newList->status  == 1 ? 'selected':'' }}>Publish</option>
                                    <option value="0" {{ $newList->status  == 0 ? 'selected':'' }}>Draft</option>
                                </select>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

            </div>

        <div class="col-lg-12 col-sm-12">

            @if($newList->image_news == 1)
            <div class="card-header">
                <div class="row">

                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div style="text-align: right;">
                            <button type="button" data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg"class="btn btn-primary btn-sm">Copy Image Link For Upload</button>
                        </div>
                    </div>

            </div>
            </div>

            @endif


        <div class="card">
            <div class="card-body">

                <div class="col-lg-12 col-sm-12">
                    <div class="form-group">
                        <label class="fw-700 fs-16 form-label">News Description <span style="color:red;">*</span></label>
                        <textarea required class="form-control summernote" name="des" placeholder="Description" required>
                            {!! $newList->des !!}
                        </textarea>
                    </div>
                </div>

            </div>
        </div>

        </div>

    </div>

    <div class="form-actions mt-10">
        <button type="submit"  class="btn btn-primary"> <i class="fa fa-check"></i>Update</button>
    </div>
</form>
<input type="hidden" id="editId" value="0"/>
</section>
</div>
</div>
<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myLargeModalLabel">Image Link List</h4>
                <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">

                <div class="table-responsive">

                    <table id="" class="table table-bordered table-hover ">
                        <thead>
                            <tr>
                                <th>Sl</th>
                                <th>Image</th>
                                <th>Image Link</th>
                            </tr>
                            </thead>
                            <tbody>
                                @foreach($imageLinkList as $key=>$unitLists)
                            <tr>
                                <td>{{ $key+1 }}</td>

                                <td> <img src="{{ asset('/') }}{{$unitLists->image_folder  }}" style="height:50px;"/></td>

                                <td>

                                  <span id="copyValue{{$unitLists->id}}">  {{url('')}} {{'/'.$unitLists->image_folder}}</span>
                                  <br>
                                  <button id="copyLink{{$unitLists->id}}" class="btn btn-info btn-sm">copy</button>

                                </td>



                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>

            </div>
        </div>
    </div>
</div>
@endsection

@section('script')
<script>

    //priority status  code


    $(document).on('change', '.inventoryId', function () {

        var name_id = $(this).find(':selected').val();
        var id =   $(this).find(':selected').data('mid');
        var lid =   $(this).find(':selected').data('lid');
        var unit =   $(this).find(':selected').data('unit');
        var quantity =   $(this).find(':selected').data('quantity');

        $('#avq'+lid).html('Available Quantity: '+ quantity + ' '+unit );


        // $.ajax({
        // url: "{{ route('getInventoryInfo') }}",
        // method: 'get',
        // data: {name_id:name_id,id:id,unit:unit,quantity:quantity},
        // beforeSend: function(){
        // $('#loader').show()
        // },
        // complete: function(){
        // $('#loader').hide()
        // },
        // success: function(data) {

        // alertify.set('notifier','position','top-center');
        // alertify.success('Prioriry Updated SuccessFully');
        // location.reload(true);


        // }
        // });

    });
    $(document).on('keyup', '.productQuantity', function () {
        var id =   $(this).data('qid');
        var quantity = $('#m'+id).find(':selected').data('quantity');

        if( $(this).val() > quantity){

            alertify.alert('Error', 'Quantity Not Available!', function(){ alertify.success('Ok'); });

            $('#finalSubmitButton').hide();

        }else{

            $('#finalSubmitButton').show();

        }

        //alert(quantity);
    });
    </script>

<script>
    $("#stock_type").change(function(){
            if(this.value === 'daily' || this.value === 'fixed') {
               $("#product_stock_div").show()
            }
            else {
                $("#product_stock_div").hide()
            }
        });
</script>
<script>

        $(document).on('change', '#category_id', function () {

            var value = $(this).find(':selected').val();


                $.ajax({
            url: "{{ route('getSubCategoryId') }}",
            method: 'get',
            data: {value:value},
            beforeSend: function(){
                $('#loader').show()
            },
            complete: function(){
                $('#loader').hide()
            },
            success: function(data) {


                $('#subcategory_id').html('');
                $('#subcategory_id').html(data);
                alertify.set('notifier','position','top-center');
                alertify.success('SubCategory Viewed');
                //location.reload(true);


            }
            });

    });

    </script>


<script>
    imgInp.onchange = evt => {
  const [file] = imgInp.files
  if (file) {
    blah.src = URL.createObjectURL(file)
  }
}
</script>

@endsection
